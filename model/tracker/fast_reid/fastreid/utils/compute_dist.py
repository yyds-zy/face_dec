...
@torch.no_grad()
def compute_euclidean_distance(feat_1: torch.Tensor, feat_2: torch.Tensor) -> np.ndarray:
    m, n = feat_1.size(0), feat_2.size(0)
    dist = (
        feat_1.pow(2).sum(1, keepdim=True).expand(m, n) +
        feat_2.pow(2).sum(1, keepdim=True).expand(n, m).t()
    )
    dist -= 2 * torch.mm(feat_1, feat_2.t())
    return dist.cpu().numpy()


@torch.no_grad()
def compute_cosine_distance(feat_1: torch.Tensor, feat_2: torch.Tensor) -> np.ndarray:
    feat_1 = F.normalize(feat_1, p=2, dim=1)
    feat_2 = F.normalize(feat_2, p=2, dim=1)
    cosine_sim = torch.mm(feat_1, feat_2.t())
    dist = 1 - cosine_sim
    return dist.cpu().numpy()
...
def build_dist(feat_1: torch.Tensor, feat_2: torch.Tensor, metric: str = "euclidean", **kwargs) -> np.ndarray:
    """
    Compute distance matrix between two batches of features.

    Args:
        feat_1 (torch.Tensor): shape [N, D]
        feat_2 (torch.Tensor): shape [M, D]
        metric (str): 'euclidean' or 'cosine'

    Returns:
        np.ndarray: [N, M] distance matrix
    """
    assert metric in ["euclidean", "cosine"], f"Unsupported metric: {metric}"
